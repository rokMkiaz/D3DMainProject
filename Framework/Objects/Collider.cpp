...
void Collider::Update()
{
	...
	temp.Position(&bounding.Position);//월드의 이동된 위치
...
bool Collider::Intersection(Ray& ray, float* outDistance)
{
	*outDistance = 0.0f;
	...
	D3DXVec3TransformCoord(&minPosition, &lines[0], &world); //min
	D3DXVec3TransformCoord(&maxPosition, &lines[7], &world); //max

	if (fabsf(ray.Direction.x) == 0.0f) ray.Direction.x = 1e-6f; //최소 초기값
